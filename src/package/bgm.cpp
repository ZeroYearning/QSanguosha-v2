...
public:
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *lvmeng, QVariant &data) const
    {
        if (triggerEvent == GameStart) {
            lvmeng->gainMark("@wu");
            room->handleAcquireDetachSkills(lvmeng, "jiang|nosqianxun");
        } else if (data.toString() == "mouduan") {
            if (lvmeng->getMark("@wu") > 0)
                room->handleAcquireDetachSkills(lvmeng, "jiang|nosqianxun");
            else if (lvmeng->getMark("@wen") > 0)
                room->handleAcquireDetachSkills(lvmeng, "nosyingzi|keji");
        }
        return false;
    }
};
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == CardsMoveOneTime) {
            ...
            if (move.from == player && player->isAlive() && player->hasSkill(this, true)
                ...
                player->loseMark("@wu");
                player->gainMark("@wen");
                room->handleAcquireDetachSkills(player, "-jiang|-nosqianxun|nosyingzi|keji", true);
            }
        } else if (player->getPhase() == Player::RoundStart && lvmeng && lvmeng->getMark("@wen") > 0
            && lvmeng->canDiscard(lvmeng, "he") && room->askForCard(lvmeng, "..", "@mouduan", QVariant(), objectName())) {
            if (lvmeng->getHandcardNum() > 2) {
                ...
                lvmeng->loseMark("@wen");
                lvmeng->gainMark("@wu");
                room->handleAcquireDetachSkills(lvmeng, "-nosyingzi|-keji|jiang|nosqianxun", true);
            }
        }
        return false;
    }
...
public:
    ...
    void onSkillDetached(Room *room, ServerPlayer *player) const
    {
        if (player->getMark("@wu") > 0) {
            player->loseMark("@wu");
            room->handleAcquireDetachSkills(player, "-jiang|-nosqianxun", true);
        } else if (player->getMark("@wen") > 0) {
            player->loseMark("@wen");
            room->handleAcquireDetachSkills(player, "-nosyingzi|-keji", true);
        }
    }
};
